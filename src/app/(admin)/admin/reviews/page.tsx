...
export default function AdminReviewsPage() {
  ...
  useEffect(() => {
    fetch("/api/reviews?limit=50&hidden=false", { credentials: "include" })
      .then(r => r.json())
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-gray-500">Manage customer reviews and respond</p>
      </div>

      {loading ? (
        <div className="space-y-4">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-28 animate-pulse rounded-2xl bg-gray-100" />)}</div>
      ) : reviews.length === 0 ? (
        ...
        <div className="space-y-4">
          {reviews.map(r => (
            <div key={r._id} className={`rounded-2xl border bg-white p-5 shadow-sm ${r.isHidden ? "opacity-60" : ""}`}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    <div className="flex">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <FaStar key={i} size={13} className={i < r.rating ? "text-amber-400" : "text-gray-200"} />
                      ))}
                    </div>
                    <span className="text-sm font-medium text-gray-700">{r.user?.name ?? r.user?.firstName ?? r.user?.email}</span>
                ...
                <div className="flex flex-shrink-0 gap-2">
                  ...
                  <button onClick={() => hideReview(r._id, !r.isHidden)} disabled={updating === r._id}
                    className={`rounded-lg px-3 py-1.5 text-xs font-medium text-white transition disabled:opacity-50 ${
                      r.isHidden ? "bg-green-600 hover:bg-green-700" : "bg-red-500 hover:bg-red-600"
                    }`}>
              ...
              {responding === r._id && (
                <div className="mt-4 flex gap-2">
                  <textarea
                    ...
                    rows={2}
                    className="flex-1 rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 resize-none"
                  />
                  <button onClick={() => submitResponse(r._id)}
                    className="rounded-xl bg-blue-700 px-4 text-sm font-semibold text-white hover:bg-blue-800 transition">
                    Send
                  </button>
                </div>
              )}
