...
import {
  MapPinIcon, Clock01Icon, UserGroupIcon,
  CheckmarkCircle01Icon, ArrowLeft01Icon, Calendar03Icon,
...
interface Package {
  ...
  maxTravelers: number; minTravelers?: number;
  includedServices: string[]; excludedServices?: string[];
...
export default function PackageDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/packages/${slug}`, { signal: controller.signal })
      .then(r => r.json())
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="mb-8 overflow-hidden rounded-2xl shadow-sm">
          <div className="relative h-64 w-full bg-gradient-to-br from-green-100 to-teal-200 sm:h-80">
            {pkg.coverImage
              ? <Image src={pkg.coverImage} alt={pkg.title} fill className="object-cover" sizes="100vw" priority />
              : <div className="flex h-full items-center justify-center"><MapPinIcon size={52} className="text-teal-200" /></div>
            }
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent" />
            <div className="absolute bottom-0 left-0 right-0 p-6">
              {pkg.isPromotional && <span className="mb-2 inline-block rounded-full bg-red-500 px-3 py-1 text-xs font-bold text-white">SALE</span>}
              <h1 className="text-2xl font-bold text-white line-clamp-2 sm:text-3xl">{pkg.title}</h1>
              <div className="mt-1.5 flex flex-wrap gap-3 text-sm text-white/80">
                ...
                {pkg.averageRating > 0 && (
                  <span className="flex items-center gap-1"><FaStar size={11} className="text-amber-400" />{pkg.averageRating.toFixed(1)} ({pkg.totalReviews})</span>
                )}
              </div>
            </div>
          </div>
                      ...
                      <div key={day.day} className="overflow-hidden rounded-xl border border-gray-200">
                        <button onClick={() => setOpenDay(openDay === day.day ? null : day.day)}
                          ...
                          <div className="flex items-center gap-4">
                            <span className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${openDay === day.day ? "bg-blue-700 text-white" : "bg-blue-100 text-blue-700"}`}>{day.day}</span>
                          ...
                          <div className="border-t border-gray-100 px-5 py-4">
                            ...
                            {(day.activities ?? []).length > 0 && (
                              <div className="mt-3 flex flex-wrap gap-2">
                                {day.activities!.map(a => <span key={a} className="rounded-full bg-blue-50 px-2.5 py-1 text-xs text-blue-700">{a}</span>)}
                              </div>
                            )}
                          </div>
                        )}
          ...
          <div className="space-y-5 lg:sticky lg:top-24 h-fit">
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-3 text-sm text-gray-600 mb-5">
                ...
                {pkg.difficultyLevel && <div className="flex items-center gap-2"><span className="text-blue-500">⚡</span><span className="capitalize">{pkg.difficultyLevel} difficulty</span></div>}
              </div>
              <button onClick={() => router.push(`/booking/${pkg.slug}`)}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-700 py-3 text-sm font-semibold text-white transition hover:bg-blue-800">
